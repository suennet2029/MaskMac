...
3. **Brightness**：在菜单栏下方弹出精致紧凑的毛玻璃微浮窗，通过滑块实时拖拽调节 Mac 屏幕亮度与 MacBook 原生键盘背光；
4. **Blink**：自动感知本地运行的 **Claude Code**、**CodeX** 与 **antiGravity**，联动 Mac 键盘物理 **中/英切换键（Caps Lock 绿色硬件 LED 指示灯）** 闪烁指示，任务结束后自动熄灭。
...
* ☀️ **便捷亮度控制 (Brightness)**：点击状态栏 `Brightness`，无需进入系统偏好设置即可通过滑块平滑调节内建屏幕与物理键盘背光亮度。
* 💡 **AI 助手运行硬件指示 (Blink)**：启动 Claude Code、CodeX 或 antiGravity 后，MacBook 键盘上的中/英按键灯自动律动闪烁，任务完成瞬间熄灭，直观掌握执行状态。
...
| **Duo** | — | 开关开合传感器磨砂过渡特效 |
| **Brightness** | `⌘ B` | 呼出紧凑毛玻璃浮窗，滑块调节屏幕与键盘亮度 |
| **Blink** | — | 开关键盘中/英按键指示灯（感知 Claude Code、CodeX、antiGravity） |
...
本项目调用 macOS 私有显示与 HID 接口。软件按“原样”提供，作者不对任何设备异常或因硬件兼容性产生的问题承担责任。
