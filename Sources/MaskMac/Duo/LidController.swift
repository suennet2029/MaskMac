...
final class LidController: ObservableObject {
    ...
    private let preferences: DuoPreferences
    ...
    init(preferences: DuoPreferences) {
    ...
    func runPreview() {
        guard !isSuspended, preview == nil, !isActive else { return }
        if pollTimer == nil {
    ...
    private func wantsEffect(angle: Double) -> Bool {
        guard (preferences.isEnabled || preview != nil), builtInLayout.displayID != nil else { return false }
    ...
    private func reconcile(angle: Double) {
        guard (preferences.isEnabled || preview != nil), !isSuspended else { return }
